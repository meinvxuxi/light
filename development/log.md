...
## 版本 4.9（2026-09-06）
- 🎨 你画我猜上线（自定义/随机出词、限时绘制、画作墙）；QA 卡题修复；默契组合自定义名。

### 修改文件
- server.js
- public/pair.html / sync.html

### 修改原因
1. 🎨 你画我猜（双人默契空间，成员准备即开，自动轮流画者/猜者）：
   - 出词：画者自定义输入 1~12 字，或"随机出词"先预览、可换一个、确认后才进入 60 秒绘制；词只发给画者，猜者仅见字数；
   - 画布：画笔/橡皮切换、9 色、画笔 3 档粗细、橡皮 3 档大小、清空、上一步；笔画矢量 strokes 实时同步猜者；中途刷新按角色恢复对局；
   - 猜者不限次数实时猜（答对 +5 默契分，超时揭晓词），每局自动存画作墙（最终画作缩略回放 + 原词 + 猜词完整记录，成员/访客可看）；
2. 🛠 修复默契问答"准备中"卡死：开局先广播状态再发题目（此前题目先到被丢弃）；
3. 🚪 对局中移除"中止"：退出需确认（会终止本局），对方退出/掉线时留守方提示"对方已退出，游戏已终止"；
4. ✏️ 默契组合自定义名：成员一方申请改名（1~10 字）→ 对方同意生效；主页与默契空间标题显示组合名并附小字"xx × xx"；pending 可同意/拒绝/覆盖；数据随 PERSIST_SYNC（默认内存，正式开 true 才落盘）。

### 当前版本
- 4.9（上个版本 4.8，+0.1）

## 版本 4.8（2026-09-06）
- 🤝 默契空间第一版上线；主题系统支持玩家3双主题（pomelo）；留言板排序与系统消息主题色调整。

### 修改文件
- server.js
- public/sync.html（新建）/ public/pair.html（新建）
- public/lobby.html / messages.html / settings.html
- 全部 11 个页面主题引擎（pomelo 注册、玩家名主题色）

### 修改原因
1. 🤝 默契空间 v4.8（数据默认内存模式，PERSIST_SYNC=true 正式落盘）：
   - 主页 sync.html：两行 6 组两两默契值（轻文字 UI，称号在默契分前），双击进入双人默契空间；
   - 双人空间 pair.html：成员进入会话、准备制小游戏（0/2 实时计数、可取消、同刻只备一个、退出自动撤）、默契问答（题库 16 题、5 题/轮，一致 →「心有灵犀！」主题色淡透明提示 +2 分）；结束后无"再来一轮"改返回菜单；
   - 默契分累计解锁称号：初次邂逅 → 渐入佳境 → 心有灵犀 → 灵魂搭档 → 天作之合；解锁时系统自动在玩家留言板发祝贺（"系统"主题色）；
   - 任意账号可作访客进入组合默契空间只读查看（默契分/称号/画作墙；画作墙结构与读取接口已就绪，等你画我猜实装后写入）；成员可查看/返回，访客退出不影响成员对局；
   - 含测试者的组合仅内存体验、不落盘不发祝贺。
2. 🎨 主题：玩家3 新增第二专属主题 pomelo（p3/nene.jpg 桌面 + 手机 nene2.jpg，绿系同款、遮罩 0.6）；归属机制升级为 OWNER_THEMES 多主题；默契空间两页接入主题引擎（600ms 重试/pageshow）。
3. 💬 留言板：最新消息置顶（从新到旧自上而下）；系统消息发送者标题按主题色显示。

### 当前版本
- 4.8（上个版本 4.7，+0.1）

## 版本 4.7（2026-09-06）
- 头衔可自由选择；排行榜改单局最高分并含游客；个人空间标签化与测试通道；快艇游玩/结算 UI 优化；房间全员离线重置与"取消对局"。

### 修改文件
- server.js
- public/lobby.html / settings.html / yahtzee.html / test-panel.html
- public/profile.html / public/leaderboard.html（本版本内持续修改）

### 修改原因
1. 🎖 头衔系统升级：头衔按已解锁品质"收集"（隐藏成就也解锁「快艇怪人」）；默认不佩戴，可在设置页从已解锁头衔中自由选择（users.json 存档；测试账号可体验不落盘）；
2. 🏆 排行榜：快艇榜改为"单局最高分"，真实对局记录游客分数（内存模式）；个人空间战绩标签化——总览只显场次/冠军，可切各游戏看完整战绩+模式明细+成就详情（总览成就改纯色文字一行排）；成就次数/首次时间移至游戏模块与荣誉墙；
3. 👤 测试个人空间：测试账号开发者工具可"生成模拟战绩"，个人空间对自己可见（仅内存，不进任何正式数据）；
4. 🎲 快艇 UI：游玩与结算去表情包与卡片化、结算隐藏骰子区/按钮/玩家条、名次统一中文数字；计分表"选中格"半透明主题高亮（蓝/粉/绿），未选中无提示；
5. 🧹 房间：全员离线自动重置房间（含残留对局）；新增"取消对局"——对局中玩家可申请（带确认框、可撤回、显示已同意人数），全员同意后取消，不计场次/胜率/排行榜，已触发成就保留。
...
## 版本 4.6（2026-09-06）
- 持久化开关改环境变量控制；个人空间/排行榜主题适配修复；战绩分级钻取。
...
### 修改原因
1. PERSIST_ACHIEVEMENTS / PERSIST_TIMELINE 改为读环境变量（默认内存开发模式；正式部署设 PERSIST_*=true 即落盘，启动日志显示当前模式）；
2. profile / leaderboard 补 600ms 主题重试与 pageshow 重刷（此前先于身份注册拉档案，一直停在初始配色）；
3. 个人空间战绩分级：全部游戏总览（默认展开）→ 各游戏 ▸ → 各模式（人数）明细；server 返回 stats.totals + stats.byGame。
...
## 版本 4.5（2026-09-06）
- 主页功能区继续转正：新增「👤 个人空间」「🏆 排行榜」「🎖 成就头衔」。

### 修改文件
- server.js
- public/lobby.html
- public/profile.html（新建）
- public/leaderboard.html（新建）

### 修改原因
1. 👤 个人空间（正式玩家拥有，游客自动回大厅）：
   - server 新增 get_profile_page：显示名 / 头衔 / 在线与上次在线时间 / 战绩（场次、冠军、胜率、累计总分、单局最高，由时光墙比赛记录汇总）/ 已解锁成就（按品质排序：名称、首次时间、次数）；
   - 新建 profile.html：轻文字排版（非卡片），顶栏返回大厅，内置五套主题引擎。
2. 🏆 排行榜（游客可进入查看）：
   - server 新增 get_leaderboard：快艇战绩榜（按累计总分排序，含局数/冠军/总分）+ 成就榜（按解锁成就数排序）；
   - 新建 leaderboard.html：双标签切换，前三名高亮，内置五套主题引擎。
3. 🎖 成就头衔：
   - 规则：以该玩家已解锁成就的最高品质定级——common「快艇新秀」/ rare「快艇好手」/ epic「快艇高手」/ legend「快艇大师」/ 仅隐藏成就时「快艇怪人」；
   - server 统一 playerTitle() 计算；online_users 广播列表带 title；
   - 大厅状态卡名字旁显示头衔小标签（粉/绿主题下同步变色）；个人空间与排行榜行内同步展示。
4. 大厅接线：功能「个人空间/排行榜」由"施工中"提示改为真实跳转；正式玩家**双击状态卡**直达对应玩家的个人空间（游客双击无效）。
